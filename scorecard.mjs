...
//   node scorecard.mjs <site-key> [--url http://127.0.0.1:8790] [--review-hours 14] [--cycle-days 30]
...
if (!key) { console.error('usage: node scorecard.mjs <site-key> [--url URL] [--review-hours N] [--cycle-days N]'); process.exit(1); }
...
const list = await (await fetch(BASE + '/v1/records?limit=100', {headers: H})).json();
if (list.error) { console.error('error: ' + list.error); process.exit(1); }
const audit = await (await fetch(BASE + '/v1/audit/verify')).json();
const recs = list.records.slice().sort((a, b) => a.received_at.localeCompare(b.received_at));
